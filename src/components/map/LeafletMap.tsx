...
type LeafletMapProps = {
  ...
  points?: GeoJSONTypes.FeatureCollection<GeoJSONTypes.Point, { id?: string; name?: string; diapersDistributed?: number }>;
          ...
          <>
            {points.features.map((f, i) => {
              ...
              const colorForCount = (c: number) => {
                if (c >= 100000) return { color: "#2bb30f", fillColor: "#39FF14" }; // neon
                if (c >= 50000) return { color: "#cc0000", fillColor: "#ff4d4d" }; // red
                if (c >= 10000) return { color: "#008f00", fillColor: "#00cc00" }; // green
              ...
              const circle = (
                <CircleMarker
                  ...
                  center={[lat, lng]}
                  radius={radius}
                        pathOptions={{ color: circleColors.color, fillColor: circleColors.fillColor, fillOpacity: 0.5 }}
                />
              );
