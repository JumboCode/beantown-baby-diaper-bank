...
export default function ChoroplethMap ({ width, height, geojson, data }: ChoroplethProps) {
  const[hoveredData, setHoveredData] = useState<PopOverData | null> (null);
  const[popOverPosition, setPopOverPosition] = useState<PopOverCoordinates | null>(null);
  ...
  if (!geojson || !geojson.features || !pathGenerator) {
    return <svg width={width} height={height}><text>Loading or Error...</text></svg>;
  }
    ...
    <div>
      ...
      >
        ...
        <Popover.Dropdown>
          {hoveredData && (
            <div>
              <strong>{hoveredData.townName}</strong><br />
