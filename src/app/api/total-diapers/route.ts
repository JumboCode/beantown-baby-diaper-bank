...
export async function GET(request: Request) {
  try {
    ...
    if (year) {
      ...
      const totalDiapers =
        cumulativeResults._sum.numDiapers == null
          ? 0
          : Number(cumulativeResults._sum.numDiapers);
      const yearlyTotalDiapers =
        yearlyResults._sum.numDiapers == null
          ? 0
          : Number(yearlyResults._sum.numDiapers);

      return NextResponse.json({ totalDiapers, yearlyTotalDiapers });
    }
