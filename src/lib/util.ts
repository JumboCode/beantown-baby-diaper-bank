...
export function parseDistributionData(csv: string) {
  ...
  while (i < data.data.length) {
    ...
    if (row[0] != '') { // name exists
      let total_diapers_distributed = Number(row[2].replace(",", ""));
      let total_children_helped = Number(row[1].replace(",", ""));
      ...
      provider = {
        ...
        cities: [{
          ...
          diapers: (Number(row[4].replace("%", "")) / 100) * total_diapers_distributed,
          children_helped: (Number(row[4].replace("%", "")) / 100) * total_children_helped
        }]
      };
